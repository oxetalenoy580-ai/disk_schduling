...
function drawCircle(x, y, radius, color, glowColor, glowSize) {
    ctx.save();
    ctx.beginPath();
...
function drawDiskPathLoc(arr) {
    ...
    ctx.stroke();
    ctx.restore();
    const trackGrad = ctx.createLinearGradient(padding, 0, app.width - padding, 0);
...
function drawAL(arr) {
    ...
    function animate(timestamp) {
        ...
        const valEl = pathEl.querySelector(".stat-value");
        if (valEl) {
...
function calculateTotalLen(arr) {
    ...
    const valEl = distEl.querySelector(".stat-value");
    if (valEl) {
